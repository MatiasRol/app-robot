...
type MapPointConfirmVariant = 'navigate' | 'route';

interface MapPointConfirmModalProps {
  visible: boolean;
  variant: MapPointConfirmVariant;
  onConfirm: () => void;
  onCancel: () => void;
}

export function MapPointConfirmModal({
  visible,
  variant,
  onConfirm,
  onCancel,
}: MapPointConfirmModalProps) {
  const title =
    variant === 'navigate'
      ? '¿DESEA COLOCAR\nESTE PUNTO DE\nNAVEGACIÓN?'
      : '¿DESEA AGREGAR\nESTE WAYPOINT?';
